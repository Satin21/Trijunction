...
def base_ham(trijunction, f_params, parameters):
    ...
    ham = trijunction.hamiltonian_submatrix(sparse=True, params=f_params(**parameters))
...
def diagonalisation(new_param, kwant_params, voltages, params, nevals=20):
    """ """
    trijunction, f_params, linear_terms = kwant_params
    params.update(new_param)

    linear_ham = sum(
        [voltages[key] * linear_terms[key] for key, value in voltages.items()]
    )
