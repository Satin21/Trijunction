...
def two_sided(system, l_cut):
    ...
    PL = kwant.operator.Density(
        system, where=lambda site: site.pos[1] < 0 or site.pos[1] >= l_cut, sum=True
    ).tocoo()
    PR = kwant.operator.Density(
        system, where=lambda site: l_cut > site.pos[1] >= 0, sum=True
    ).tocoo()
    return PL, PR
...
def one_sided(system):
    # Projectors of each subspace
    PL = kwant.operator.Density(
        system, where=lambda site: site.pos[1] < 0, sum=True
    ).tocoo()
    PR = kwant.operator.Density(
        system, where=lambda site: site.pos[1] >= 0, sum=True
    ).tocoo()
...
def lowdin_partition(system, hamiltonian, sides, l_cut):
    if sides == 1:
...
def majorana_dot_basis(
    system, f_params_potential, potential, params, n_dot, n_extra, sides, l_cut
):
    orthogonal_evecL = []
    ...
    for w in wires_isolated:
        ...
        f_potential = f_params_potential(
            potential=get_potential(potential), params=params
        )
        hamiltonian = system.hamiltonian_submatrix(params=f_potential, sparse=True)
...
def prepare_perturbation(
    ...
    perturbed_hamiltonian = system.hamiltonian_submatrix(
        params=f_potential, sparse=True
    )
...
def calculate_effective_models(H0, Hgate, Hcoup, A_basis, B_basis):
    def solver(index, order):
        eff = effective.effective_model(
            H0=H0,
            H1={"gate": Hgate[index], "p": Hcoup[index]},
            evec_A=A_basis,
            evec_B=B_basis,
            order=order,
