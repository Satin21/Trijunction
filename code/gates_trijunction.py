...
def gates_trijunction(area, angle, wire_width, gap, extra_width):
    ...
    sides_width = triangle_width/2 - (middle_width/2 + 2*gap + tunnel_width)

    screening_centers = np.array(
        [
            [0, tunnel_length/2],
            [triangle_width/4 + tunnel_width/2 + gap + sides_width/2, tunnel_length/2],
            [-(triangle_width/4 + tunnel_width/2 + gap + sides_width/2), tunnel_length/2],
    ...
    screening_gate_right = np.array(
        [
            [triangle_width/2, tunnel_length + gap * (3/2) + gap * np.tan(angle)],
            [triangle_width/2, total_length],
            [tunnel_width/2 + gap, total_length],
            [tunnel_width/2 + gap, total_length - tunnel_length - gap/2 ],
    ...
    screening_gates = {'left_triangle_screen_gate': screening_gate_left,
                       ...
                       'bot_central_screen_gate': rectangular_gate(center=screening_centers[0],
                                                                   length=screening_length,
                                                                   width=middle_width),
                       'bot_left_screen_gate': rectangular_gate(center=screening_centers[1],
                                                                   length=screening_length,
                                                                   width=sides_width),
                       'bot_right_screen_gate': rectangular_gate(center=screening_centers[2],
                                                                   length=screening_length,
                                                                   width=sides_width),
